...
export async function createProduct(data, userId) {
  await connectDB();
  const product = await Product.create({ ...data, creator: userId, status: "pending" });
  return product;
}
...
export async function getRandomProduct(excludeIds = []) {
  ...
  if (count === 0) throw new ApiError("No products available", 404);
...
// Dislike Algorithm
/**
 * Records a dislike and adjusts the product score.
 * Also updates the user's disliked list so future recommendations
 * can filter similar tags/categories.
 */
